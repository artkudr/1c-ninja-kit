...
```powershell
powershell.exe -NoProfile -File "%USERPROFILE%\.cursor\skills\form-validate\scripts/form-validate.ps1" -FormPath "Catalogs/Номенклатура/Forms/ФормаЭлемента"
powershell.exe -NoProfile -File "%USERPROFILE%\.cursor\skills\form-validate\scripts/form-validate.ps1" -FormPath "src/МояОбработка/Forms/Форма/Ext/Form.xml"
```
