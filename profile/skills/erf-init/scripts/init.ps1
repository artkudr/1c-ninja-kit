...
param(
	...
	# Каталог-корзина (обычно src/erf). Скрипт пишет nested: SrcDir/<Name>/<Name>.xml + SrcDir/<Name>/<Name>/
	[string]$SrcDir = "src/erf",
...
# Nested: src/erf/<Name>/<Name>.xml + src/erf/<Name>/<Name>/…
$objectRoot = Join-Path $SrcDir $Name
$rootFile = Join-Path $objectRoot "$Name.xml"
$reportDir = Join-Path $objectRoot $Name
...
if (Test-Path $reportDir) {
	Write-Error "Каталог уже существует: $reportDir"
	exit 1
}
