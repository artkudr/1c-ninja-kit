...
/** Default 1C web-client locale (path segment). Explicit /en_US/ etc. are left alone. */
const DEFAULT_WEB_LOCALE = 'ru_RU';
const WEB_LOCALE_RE = /^[a-z]{2}(_[a-z]{2})?$/i;
const WEB_SERVICE_ROOTS = new Set(['hs', 'odata', 'ws', 'e1cib']);

/**
 * Ensure web-client URL has a locale segment (`/ru_RU/`) when missing.
 * Does not override an explicit locale (en_US, ru, …) or service paths (/hs/, /odata/, …).
 */
export function ensureWebClientLocale(url, locale = DEFAULT_WEB_LOCALE) {
  let u;
  try {
    u = new URL(url);
  } catch {
  ...
  if (parts.length >= 2) {
    ...
    if (WEB_SERVICE_ROOTS.has(second.toLowerCase())) return url;
    if (WEB_LOCALE_RE.test(second)) return url;
  }
  if (parts.length === 1) {
    u.pathname = `/${parts[0]}/${locale}/`;
    return u.href;
  }
  return url;
}

const PW_LOCALE = 'ru-RU';

/**
 * Find the 1C browser extension in Chrome/Edge user profiles.
 * Returns the path to the latest version, or null if not found.
 * Can be overridden via `extensionPath` in connect/session options.
 */
