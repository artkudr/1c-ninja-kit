...
### Вариант 3: явный путь (общий на машину)

Чтобы не копировать ffmpeg в каждый проект, передайте путь параметром:
...
Или задайте переменную окружения `FFMPEG_PATH`.

### Порядок поиска ffmpeg

1. `opts.ffmpegPath` — явный путь (параметр вызова или env)
...
### Configuration (передать в `addNarration` opts)

```js
await addNarration(video.file, {
  provider: 'edge',
...
// Add narration (TTS opts передаются явно)
