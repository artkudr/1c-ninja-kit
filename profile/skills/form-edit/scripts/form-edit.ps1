...
# read-only configs unless allowed. Trigger = bin present; mode always deny.
# Guard errors degrade to allow.
...
function Assert-EditAllowed([string]$targetPath, [string]$require) {
	try {
		...
		$offNote = "Чтобы править объект на поддержке: снять с поддержки через /support-edit или не трогать объект на замке."
