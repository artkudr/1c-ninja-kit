...
description: >-
  Составление корректных запросов на языке запросов 1С для live_query
  (1c-ninja-mcp) или execute_query (1c-mcp-toolkit, fallback).
  Синтаксис ВЫБРАТЬ/ИЗ/ГДЕ, виртуальные таблицы, JOIN, временные таблицы,
  параметры. Используй при проверке запросов в живой ИБ или выборке данных.
---
...
**Выполнение в живой ИБ (приоритет):** MCP **1c-ninja-mcp** → `live_query` (при необходимости `live_validate_query`, `live_metadata_*`). Скилл `1c-ninja-mcp`.  
**Fallback:** `execute_query` в **1c-mcp-toolkit**, если BSL Analyzer / публикацию поднять нельзя (скилл `1c-mcp-toolkit`, EPF на `:6003`).
...
Before composing a query, if you are not certain about the exact names, **use live metadata tools or any other available tool** to retrieve 1C configuration metadata:
- Prefer `live_metadata_list` / `live_metadata_structure` (1c-ninja-mcp)
- Fallback: `get_metadata` (1c-mcp-toolkit)
- Discover: catalogs, documents, registers; attributes; tabular sections; register dimensions/resources

**Workflow:** retrieve metadata → confirm names → compose query → `live_query` (или `execute_query`)
...
Every 1C metadata object has a fixed query-name pattern. Use exact names from `live_metadata_*` / `get_metadata`.
...
Passed via `live_query` parameter `parameters` (JSON-строка) или `execute_query` params: `{"Маска": "%Рога%"}`.
