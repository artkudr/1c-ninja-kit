...
| SrcDir        | нет          | `src/epf`    | Корзина исходников (nested: `SrcDir/<Имя>/<Имя>/…`) |
...
1. Найди `ObjectModule.bsl` через Glob: `src/epf/{{ProcessorName}}/{{ProcessorName}}/Ext/ObjectModule.bsl` (канон nested; допускается также `src/erf/…` для отчётов)
2. Прочитай файл
