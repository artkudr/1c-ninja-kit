...
param(
	...
	# Каталог-корзина (обычно src/epf). Скрипт пишет nested: SrcDir/<Name>/<Name>.xml + SrcDir/<Name>/<Name>/
	[string]$SrcDir = "src/epf",
...
# Nested: src/epf/<Name>/<Name>.xml + src/epf/<Name>/<Name>/…
$objectRoot = Join-Path $SrcDir $Name
$rootFile = Join-Path $objectRoot "$Name.xml"
$processorDir = Join-Path $objectRoot $Name
...
if (Test-Path $processorDir) {
	Write-Error "Каталог уже существует: $processorDir"
	exit 1
}
...
Write-XmlFile (Join-Path ((Resolve-Path $objectRoot).Path) "$Name.xml") $xml $enc
